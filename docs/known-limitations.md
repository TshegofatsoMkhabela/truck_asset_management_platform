...
| JWT revocation before expiry | Not built. A JWT (JSON Web Token) issued at login remains valid for its full lifetime (default 60 minutes) even if the account is later disabled or the token is otherwise compromised; there is no logout endpoint or denylist | The stateless design chosen in [ADR 0003](adr/0003-authentication-and-rbac.md) needs no shared session store across container instances, but early revocation would require adding one (e.g. a short-lived token plus a refresh flow, or a denylist cache). Not built because no requirement calls for it and the token lifetime is short enough for a demo session |
