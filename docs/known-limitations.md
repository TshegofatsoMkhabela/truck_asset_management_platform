...
| Orchestrator's Swagger UI not reachable in the containerized stack yet | `springdoc-openapi` and the JWT security scheme it needs land in #9, which is not on `main`. `docker compose up` today serves `/swagger-ui.html` as a 404. matching-service's FastAPI-generated equivalent (`/docs`) already works, since it needs no extra dependency | No action needed beyond #9 merging; #8's own smoke test (`docs/testing-summary.md` T-25) is written to be re-run once it does, rather than assumed to pass |
