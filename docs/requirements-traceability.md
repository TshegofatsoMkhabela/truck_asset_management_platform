...
|---|---|---|---|---|---|
| FR-01 | Role-based registration/login for Freight Owner, Transporter and Admin | Must | `POST /auth/register`, `POST /auth/login` (`AuthController`) issue a real signed JWT naming the user's id and role; library choice recorded in [ADR 0003](adr/0003-authentication-and-rbac.md). **Login and token issuance work; role-based access control does not yet gate anything.** Every business endpoint now overrides its caller-supplied identity field with the JWT's id when a real token is presented (`CurrentUser`), which removes retyping ids in a logged-in Swagger demo, but `SecurityConfig` still `permitAll()`s every business endpoint, so a caller presenting no token at all still gets through unauthenticated exactly as before | `AuthControllerTest`, `JwtServiceTest`, `JwtAuthenticationFilterTest`, `SecurityConfigTest` | Partial, see [Known Limitations](known-limitations.md) |
...
| FR-04 | Transporter can create and view available trucks | Must | POST/GET/PATCH /trucks (`TruckController`, #12) | `TruckControllerTest.postTruck_createsAndReturns201`, `getTruck_returnsExistingTruck`, `getTruck_notFound_returns404`, `listTrucksByTransporter_returnsMatchingTrucks`, `patchTruck_updatesStatus`, `postTruck_invalidRequest_returns400` | Complete |
